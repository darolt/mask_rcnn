...
def _overlap_idx(box1_x1, box1_x2, box2_x1, box2_x2):
    ...
    if box1_x1 >= box2_x2 or box1_x2 <= box2_x1:
        ...
        if box2_x1 <= box1_x1 <= box2_x2:
            inter1_x1 = (box2_x2 - box1_x1)/10000.0
            inter2_x1 = box1_x1 - box2_x1
        elif box1_x1 <= box2_x1 <= box1_x2:
            inter1_x1 = box2_x1 - box1_x1
            inter2_x1 = (box1_x2 - box2_x1)/10000.0
        else:
    ...
    return (inter1_x, inter2_x), has_intersect
...
def _get_intersection_idx(box1, box2):
    ...
    intersect_idx_x, has_intersect_x = _overlap_idx(box1_x1, box1_x2,
                                                    box2_x1, box2_x2)
    intersect_idx_y, has_intersect_y = _overlap_idx(box1_y1, box1_y2,
                                                    box2_y1, box2_y2)
    has_intersect = has_intersect_x and has_intersect_y
    # concat
    ...
    intersect2_idx = torch.stack(
        ...
        dim=0)
    return (intersect1_idx, intersect2_idx), has_intersect


def _compute_intersection(pred_mask, gt_mask, pred_inter_idx, gt_inter_idx):
    pred_inter_idx = pred_inter_idx.to(torch.int32).detach()
    pred_y1, pred_x1, pred_y2, pred_x2 = pred_inter_idx
    gt_y1, gt_x1, _, _ = gt_inter_idx.to(torch.int32)
...
def _compute_factor(pred_inter_idx):
    # calculate area of boxes' intersection
    intersection_area_box = ((pred_inter_idx[0]-pred_inter_idx[2]) +
                             (pred_inter_idx[1]-pred_inter_idx[3]))
    intersection_area_box = intersection_area_box.to(ExeCfg.DEVICE)
    factor = (intersection_area_box.sigmoid()) - 1.0
...
def _compute_iou(gt_box, gt_mask, pred_box, pred_mask):
    inter_idx, has_intersect = _get_intersection_idx(pred_box, gt_box)
...
def _compute_map(ious):
    ...
    for thresh_idx, threshold in enumerate(thresholds):
        hits = ((ious - threshold)*100).sigmoid()
        gt_sum, pred_sum = hits.sum(dim=0), hits.sum(dim=1)
        tp = pred_sum.sum()
        fp = (1 - gt_sum).sum()
        fn = (1 - pred_sum).sum()
